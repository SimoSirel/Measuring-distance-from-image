...
def calculate_distances(permutations, rects, width, height):
    ...
    for perm in permutations:
        ...
        average_px_meter = (width-540) / 20 #lambist proovitud arv
...
try: 
    with tqdm(total=total_frames) as pbar:
        for frame_nr, image in enumerate(video_to_images(input_video)):
            t1 = time.time()
            ...
            people_limit_in_area = 15
            cv2.putText(image, "Max allowed: "+str(people_limit_in_area), (int(Width * 0.95)-650, int(Height * 0.90)), font, 3, (255, 255, 255), 2,
                        cv2.LINE_AA)
            try:
                people_on_camera = len(coordinates)
                if people_on_camera < people_limit_in_area :
                    cv2.putText(image, str(people_on_camera), (int(Width*0.95),int(Height*0.95)), font, 3, (0, 255, 0), 2, cv2.LINE_AA)
                else:
                    cv2.putText(image, str(people_on_camera), (int(Width*0.95),int(Height*0.95)), font, 3, (0, 0, 255), 2, cv2.LINE_AA)
            ...
            print("Frame nr",frame_nr,"Time: {:.2f}ms".format(ms))
